      ...
      private static byte[] ProcessRequest(Request request)
        {
            ...
            var splitUri = request.RequestUri.Split('?');

            var relativePath = splitUri[0];
            var queryString = "";
            try {
            ...
            if (queryParams["name"] != null) {
                name = HttpUtility.HtmlEncode(queryParams["name"]);
            }
            if (queryParams["greeting"] != null) {
                greeting = HttpUtility.HtmlEncode(queryParams["greeting"]);
            }
            ...
            switch (relativePath) {
                ...
                case "/hello.html":
                    ...
                    bodyString = Encoding.UTF8.GetString(File.ReadAllBytes("hello.html"));
                    if (greeting != "") {
                        head.Append($"Set-Cookie: greeting={greeting}\r\n");
                        bodyString = bodyString.Replace("{{Hello}}", greeting);
                    }
                    if (name != "") {
                        head.Append($"Set-Cookie: name={name}\r\n");
                        bodyString = bodyString.Replace("{{World}}", name);                     
                    }
                ...
                case "/time.html":
                    ...
                    bodyString = Encoding.UTF8.GetString(File.ReadAllBytes("time.template.html"));
                    body = Encoding.UTF8.GetBytes(bodyString.Replace("{{ServerTime}}", DateTime.Now.ToString()));
                    head.Append("Content-Type: text/html; charset=utf-8\r\n");
                    head.Append($"Content-Length: {body.Length}\r\n");
                    head.Append("\r\n");
                    break;
